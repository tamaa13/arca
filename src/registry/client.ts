 ...
 *   addRoot(rootHash)  -> sends a tx to IngatRegistry, waits 1 conf
 ...
 * machine — the local index (~/.ingat/index.json) becomes optional, and the list is
 * un-ruggable. It also makes 0G Chain explicitly load-bearing (our own contract).
 *
 * Contract: contracts/src/IngatRegistry.sol (deploy with contracts/script/Deploy.s.sol).
 */
...
function toBytes32(rootHash: string): string {
  ...
  if (!/^0x[0-9a-fA-F]{64}$/.test(hex)) {
    throw new Error(
      `IngatRegistry: rootHash must be a 32-byte hex string (got "${rootHash}")`,
    );
  }
...
/**
 * TS client for the IngatRegistry contract on 0G Chain (mainnet 16661).
 *
 * @example
...
export class RegistryClient implements MemoryRegistry {
  ...
  /** The deployed IngatRegistry address this client talks to. */
  readonly address: string;

  /**
   * @param privKeyHex      The user's secp256k1 private key (signs addRoot txs).
   * @param contractAddress IngatRegistry address. Defaults to `OG.registry`
   *                        (env INGAT_REGISTRY_ADDR). Throws if both are empty.
   */
  constructor(privKeyHex: string, contractAddress: string = OG.registry) {
    if (!contractAddress) {
      throw new Error(
        "IngatRegistry not deployed — set INGAT_REGISTRY_ADDR to the deployed " +
    ...
    this.contract = new Contract(contractAddress, INGAT_REGISTRY_ABI, wallet);
  }
